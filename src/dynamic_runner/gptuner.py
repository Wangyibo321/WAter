...
class RunnerGPTuner(RunnerTemplate):
    ...
    def optimize(self):
        ...
        self.exec_whole_last = self.success_run_last[:min(int(30*self.verify_ratio), len(self.success_run_last))]
        if str(16) not in self.exec_whole_last:
            self.exec_whole_last.append(str(16))
        self.exec_whole_idx.extend(self.exec_whole_last)
        # 选中的在完整workload执行
        print(f"exec_whole_last0:{self.exec_whole_last}")
        self.exec_whole()
