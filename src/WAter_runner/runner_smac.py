    ...
    def optimize(self):
        ...
        while True:
            ...
            if self.cur_stage == 1:
                ...
                self.exec_whole_last = [config for config in self.success_run_last[:int(len(self.success_run_last)*self.verify_ratio)] if runhistory_data[int(config)-1][4] <= self.subset_default_score * 1.0 * 1000]
